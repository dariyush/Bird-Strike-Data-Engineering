# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 19:00:51 2019
...
class MyFormatter(logging.Formatter):
    ...
    def formatException(self, ei):
        ...
        sio.close()
        return s
logFileName = f"_log_BusinessRules"
...
#%%
def PrecipitationConditions( row_id, update=False ):
    ...
    try:
        ...
        if r.Conditions_Precipitation is None:
            return None  
        else:
            Conditions_Precipitation_list = [p.strip() for p in r.Conditions_Precipitation.split(',')]
        
        pcid = None
        q = f'''select top 1 PCID from [RawDataPrecipitationConditions] 
        ...
        if pcid is None:       
            columns = f"[RawData_ID], " + ', '.join([f'[{p}]' for p in Conditions_Precipitation_list])
            values = f"{r.ID}, " + ', '.join([str(1) for i in Conditions_Precipitation_list])
            q = f"INSERT INTO [RawDataPrecipitationConditions] ({columns}) VALUES ({values})"
            _ = SQLExec(q, 1)
            
        elif update:
            columns_values = ', '.join([f'{p}=1' if p in Conditions_Precipitation_list 
                                else f'{p}=0' for p in ['Fog','Snow','Rain','None']])
            q = f'''UPDATE [RawDataPrecipitationConditions] 
                    SET {columns_values} WHERE PCID={pcid}'''
            _ = SQLExec(q, 1)
...
def DoRecord(row_id):
    ...
    _ = PrecipitationConditions( row_id )
...
if __name__ == "__main__":
    ...
    with Pool( processes=40 ) as pool:
        for _ in tqdm( pool.imap_unordered(DoRecord, range(id_min,id_max+1)), 
                      total=id_max, desc="Processing Records for Business Rules" ):
            pass